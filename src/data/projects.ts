...
export const projects: Project[] = [
  {
    ...
    content: {
      pl: {
        ...
        summary:
          "NutriMind AI to produkcyjny system RAG dla dietetyków — odpowiada na pytania wyłącznie na podstawie zweryfikowanych PDF-ów medycznych i zwraca cytowane nazwy plików źródłowych. Zbudowałam go jako rozdzielony stack: Vue 3 na Netlify, FastAPI na Render, ChromaDB jako trwała baza wektorowa na dysku.\n\nDietetyk wpisuje pytanie po polsku. LangChain wyszukuje semantycznie fragmenty z zaindeksowanych wytycznych, a Gemini 2.5 Flash generuje odpowiedź ściśle ograniczoną do pobranego kontekstu — bez wiedzy z open web. Przy każdej odpowiedzi widać odznaki ze źródłami PDF.\n\nKluczowy wzorzec to BYOK (Bring Your Own Key): klucz API użytkownika trafia z localStorage w nagłówku X-Gemini-API-Key, więc backend pozostaje stateless i bez sekretów na serwerze. Przed wdrożeniem uruchamiam pipeline Ragas — metryki Faithfulness i Answer Relevancy na zestawie ground-truth.\n\nAplikacja jest live na diet-rec.netlify.app. Ten projekt pokazuje, że projektuję RAG end-to-end: ingestion PDF, retrieval, grounding promptów, automatyczną ewaluację i deploy produkcyjny.",
        challenge:
      ...
      en: {
        ...
        summary:
          "NutriMind AI is a production RAG system for dietitians — it answers questions based only on verified medical PDFs and returns cited source filenames. I built it as a decoupled stack: Vue 3 on Netlify, FastAPI on Render, ChromaDB as a persistent on-disk vector store.\n\nThe dietitian asks a question in Polish. LangChain semantically retrieves chunks from indexed guidelines, and Gemini 2.5 Flash generates an answer strictly limited to retrieved context — no open-web knowledge. Every response shows PDF source badges.\n\nThe key pattern is BYOK (Bring Your Own Key): the user's API key flows from localStorage via the X-Gemini-API-Key header, so the backend stays stateless with no secrets on the server. Before deployment I run a Ragas pipeline — Faithfulness and Answer Relevancy on a ground-truth dataset.\n\nThe app is live at diet-rec.netlify.app. This project shows I can design RAG end-to-end: PDF ingestion, retrieval, prompt grounding, automated evaluation, and production deployment.",
        challenge:
